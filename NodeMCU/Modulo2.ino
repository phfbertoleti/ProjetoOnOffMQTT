...
#define ENDERECO_MODULO_STR  "002"     //endereço deste módulo
...
char EstadoSaida3;
char EstadoSaida4;
const char* URLThingSpeak = "api.thingspeak.com";
String apiKey ="RW39BCJT276PBFH4";
char ContadorEnvioThingSpeak;
...
void EnviaTemperaturaEUmidade(void);
void VerificaSeDeveEnviarParaThingSpeak(void);
...
//Função: envia para o ThingSpeak a temperatura(ºC) e umidade
//Parametros: nenhum
//Retorno: nenhum
void EnviaTemperaturaEUmidade(void)
{  
    ...
    char UmidadeLida;
    char StringTemperatura[3];
    char StringUmidade[3];
    String postStr = apiKey;
    
    //le a temperatura (em graus Celsius)
    TemperaturaLida = (char)dht.readTemperature();  
    UmidadeLida = (char)dht.readHumidity();
    ...
    if (client.connect(URLThingSpeak, 80)) 
    { 
        postStr += "&field1=";    //campo 1 - temperatura 
        postStr += String(TemperaturaLida);  
        postStr += "&field2=";    //campo 2 - umidade
        postStr += String(UmidadeLida);
        postStr += "\r\n\r\n";

        Serial.println("Temperatura lida: ");
        Serial.println(String(TemperaturaLida));
        Serial.println("Umidade lida: ");
        Serial.println(String(UmidadeLida));
    ...
    Serial.println(" - Enviada temperatura e umidade para ThingSpeak");    
}

//Função: verifica se é o momento para enviar ddos ao ThingSpeak
//Parametros: nenhum
//Retorno: nenhum
void VerificaSeDeveEnviarParaThingSpeak(void)
{
    ContadorEnvioThingSpeak++;

    if (ContadorEnvioThingSpeak < CICLOS_LOOP_ENVIO)
        return;

    ContadorEnvioThingSpeak = 0;
    EnviaTemperaturaEUmidade();
}
...
void loop() 
{
    ...
    //verifica se deve enviar dados para ThingSpeak
    VerificaSeDeveEnviarParaThingSpeak();
